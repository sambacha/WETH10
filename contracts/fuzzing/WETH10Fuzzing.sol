...
contract MockHolder {
    constructor(address payable weth, address retriever) {
        WETH10(weth).approve(retriever, type(uint256).max);
    }
}
...
contract WETH10Fuzzing {
    ...
    function deposit(uint256 ethAmount) public {
        uint256 supply = address(weth).balance;
    ...
    function transfer(uint256 ethAmount) public {
        uint256 thisBalance = weth.balanceOf(address(this));
        uint256 holderBalance = weth.balanceOf(holder);
    ...
    function transferFrom(uint256 ethAmount) public {
        uint256 thisBalance = weth.balanceOf(address(this));
        uint256 holderBalance = weth.balanceOf(holder);
